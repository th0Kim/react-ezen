...
6. store 생성
7. subscribe 구독(store의 내장 함수) - 리액트 리덕스에서는 Provider로 사용한다.
8. 디스패치(store의 내장 함수)
...
### Hooks를 사용하여 컨테이너 컴포넌트 만들기
  ...
  const onIncrease = useCallback(() => dispatch(increase()), [dispatch]);// hooks
  const onDecrease = useCallback(() => dispatch(decrease()), [dispatch]);// hooks
...
## 클라이언트 - 미들웨어 - 서버
### 동적 컨텐츠 - '미들웨어'라고 함
  대표적으로 WAS : 백엔드에서는 Tomcat을 설치해서 사용 함.
  리액트 미들웨어 :
    "액션 -> (미들웨어) -> 리듀서 -> 스토어"
    리덕스 미들웨어는 액션을 디스패치 했을 때 리듀서에서 이를 처리하기에 앞서 사전에 지정된 작업을 실행. 액션과 리듀서의 중간자.

### 리덕스 미들웨어를 통한 비동기 작업 관리
요청 시작 -> 로딩중
요청 성공 또는 실패 시 로딩이 끝났음을 명시
...
### redux-thunk : 비동기 처리를 위한 미들웨어
  ...
  #### thunk 함수를 만들어서 디스패치 할 수 있다. 리덕스 미들웨어가 그 함수를 전달 받아 store의 dispatch와 getState를 파라미터로 넣어서 호출해준다. 구현한 thunk함수 내부에서 원하는 api요청도 하고 다른 액션을 디스패치 하거나 현재 상태를 조회하게 할 수 있음.
  예를 들면
  ```
